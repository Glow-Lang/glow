...
Sometimes, the users are just victim of the central entity's
negligence, incompetence, or change of mind,
that causes the service to be cancelled, disrupted, corrupted,
...
that can unaccountably collude against users is called a @emph{DApp},
...
DApps are both @emph{much} harder to write and @emph{much} less tolerant to mistakes.
...
@itemize[
 ...
 @item{Discord: @our-discord-channel{https://discord.com/channels/655606984514469899/655606984967585832}}
]
...
the roles of which are called @glowexp{A} and @glowexp{B}.
...
The third and fifth lines define two private variables
respectively named @glowexp{ha} and @glowexp{hb}
using the keyword @glowexp{let}, each bound to a different byte string.
The former is initially private to @glowexp{A} only,
while the latter is initially private to @glowexp{B} only.

The fourth and sixth lines use the @glowstm{publish!} statement to specify that
the respective sentences @glowexp{ha} and @glowexp{hb} will be sent
respectively on behalf of users @glowexp{A} and @glowexp{B}.
...
This contract starts with a special mandatory line @tt{#lang glow}
to indicate that it is a glow program indeed.
...
Because that's the number of changes in which participant is @emph{active}:
...
In practice, the two participants must agree @emph{off-chain} on the terms of the interaction.
