...
module ProjectOut {Identifier : Type₀} {{IsDiscrete-Identifier : IsDiscrete Identifier}}
  ...
  CtxTrans' (x₃ ∷ ptps) nothing (AST.pId name₁ {y}) = Unit
  CtxTrans' (x₃ ∷ ptps) (just (AST.pId name₂ {inl x})) (AST.pId name₁ {inl x₁}) = Empty
  CtxTrans' (x₃ ∷ ptps) (just (AST.pId name₂ {inl x})) (AST.pId name₁ {inr x₁}) = Unit
  CtxTrans' (x₃ ∷ ptps) (just (AST.pId name₂ {inr x})) (AST.pId name₁ {inl x₁}) = Unit
  ...
  CtxTrans'' : (ptps : List (Identifier × ParticipantModality))
            ...
            → Type₀
  CtxTrans'' ptps x x₁ x₂ = CtxTrans' ptps x x₁ × {!!}
 ...
 -- interactionHead (participantsWM ih) [] {_} {uniquePtcpnts ih}
  ...
  ctxTransFld : ∀ {ih : _} {hp} → (c : Context ih) (a : Stmnt ih c) →
                      CtxTrans hp c → CtxTrans hp (bindingMechanics' ih c a)
  ctxTransFld = {!!}
  ...
  paramSubst {ih} hp t =
       map-Linked'-map-Σ-Mb {D = CtxTrans {ih} hp}
         ...
         t
    where
      h : {c : Context ih} (d : CtxTrans hp c) → Stmnt ih c → Maybe (Stmnt (makeDishonest ih hp) (ctxTrans {ih} {hp} c d))
      h = {!!}
