...
-- AST.interactionHead ( x .participants) (map-List pmtIdentifierWithType (x .parameters)) {{!x .uniqueParams!}}
  {-# TERMINATING #-}
  pmtExpr : ∀ {b₁ b₂} → InteractionHead b₁ → InteractionHead b₂
  pmtExpr {b₂ = b₂} (AST.interactionHead participants₁ []) = AST.interactionHead participants₁ [] {PropMode.fromWitness' b₂ _ }
  pmtExpr {b₁} {b₂} (AST.interactionHead participants₁ (name₁ ∶ type₁ ∷ parameters₁) {yy}) = 
     let z = (pmtExpr (AST.interactionHead participants₁ (parameters₁) {PropMode.fromWitness' b₁ (proj₂ (PropMode.toWitness' b₁ yy)) }))
