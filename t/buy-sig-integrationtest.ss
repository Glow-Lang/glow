...
(def buy-sig-integrationtest
  (test-suite "integration test for ethereum/buy-sig"
    ;; Normally Buyer initiates because Buyer is the first active participant,
    ;; which can enable first-transaction-optimization.
    ...
    ;; This tests that it still works when the Seller initiates,
    ;; even though the Buyer is the first active participant,
    ;; so first-transaction-optimization doesn't work.
    ;; Seller: Alice, Buyer: Bob
    (test-case "buy sig runs successfully when passive-participant initiates"
      ...
      (def seller-balance-before (eth_getBalance alice 'latest))
      (def buyer-balance-before (eth_getBalance bob 'latest))
      ...
      (try
         ...
         (open-process
          [path: "./glow"
           arguments: ["start-interaction"
                       ...
                       ;;
                       ;; N.B. this is bob's id.
                       "--participants" "{\"Buyer\": \"0xb0bb1ed229f5Ed588495AC9739eD1555f5c3aabD\"}"
                       "--assets" "{\"DefaultToken\": \"PET\"}"
                       ]]))
       ...
       (def seller-environment
         (with-io-port proc-seller read-environment))

       (assert! (equal? seller-environment buyer-environment))
       ...
       (def signature
         (match (hash-ref buyer-environment 'signature)
           ...
           (value (error "Unexpected value for signature: " value))))
       (DBG "parsed Signature: " signature)
       (def valid? (message-signature-valid? alice signature digest))
       (def seller-balance-after (eth_getBalance alice 'latest))
       (def buyer-balance-after (eth_getBalance bob 'latest))
