(hash (* (type:arrow (@list (type:name 'Int) (type:name 'Int)) (type:name 'Int)))
      ...
      ((@make-interaction
        ...
        (switch (== (@app bitwise-and (@app bitwise-xor randA randB) 1) 0)
                ...
                (#f (block (@debug-label dlb10) (withdraw! B (@record (DefaultToken (@app * 2 wagerAmount)))) (@debug-label dlb11) (withdraw! A (@record (DefaultToken escrowAmount)))))))
       (type:arrow (@list (type:name 'Participant) (type:name 'Participant) (type:name 'Int) (type:name 'Int)) (type:tuple (@list))))
      ((@make-interaction
        ((@record (participants (@list A B)) (assets (@list DefaultToken))))
        (wagerAmount escrowAmount)
        (@debug-label dlb0)
      ...
      ((#f (block (@debug-label dlb10) (withdraw! B (@record (DefaultToken (@app * 2 wagerAmount)))) (@debug-label dlb11) (withdraw! A (@record (DefaultToken escrowAmount)))))
       (type:arrow (@list (type:name 'Bool)) (type:tuple (@list (type:record (symdict)) (type:tuple (@list))))))
      ((#t (withdraw! A (@record (DefaultToken (@app + (@app * 2 wagerAmount) escrowAmount)))))
       (type:arrow (@list (type:name 'Bool)) (type:tuple (@list (type:record (symdict)) (type:tuple (@list)))))))
