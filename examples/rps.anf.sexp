(defdata Hand
         ...
         (block (def tmp (λ (tag1) (def x5 : name (input name tag1)) x5))
                (def tmp0
                     (λ ((x6 : name))
                        (switch x6 (Rock 0) (Paper 1) (Scissors 2))))
                (def tmp1
                     (λ ((x7 : name))
                        (switch x7 (0 Rock) (1 Paper) (2 Scissors))))
...
(def NatToHand
     (λ ((x0 : int))
        ...
        (if tmp4
            Rock
            (block (def tmp5 (@app = x0 1)) (if tmp5 Paper Scissors)))))
...
(defdata Outcome
         ...
         (block (def tmp6 (λ (tag2) (def x8 : name (input name tag2)) x8))
                (def tmp7
                     (λ ((x9 : name))
                        (switch x9 (B_Wins 0) (Draw 1) (A_Wins 2))))
                (def tmp8
                     (λ ((x10 : name))
                        (switch x10 (0 B_Wins) (1 Draw) (2 A_Wins))))
...
(def NatToOutcome
     (λ ((x3 : int))
        ...
        (if tmp11
            B_Wins
            (block (def tmp12 (@app = x3 1)) (if tmp12 Draw A_Wins)))))
(def OutcomeToNat
...
(@interaction
 ...
 (def rockPaperScissors
      (λ (wagerAmount)
         ...
         (switch outcome
                 (A_Wins (def tmp22 (@app * 2 wagerAmount))
                         (withdraw! A tmp22))
                 (B_Wins (def tmp23 (@app * 2 wagerAmount))
                         (withdraw! B tmp23))
