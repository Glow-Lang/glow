...
(import
  ...
  (only-in :vyzo/libp2p/daemon daemon daemon-process-set!)
  ...
  (cond
   ...
   (else
    (let* ((path (or sock (string-append "/tmp/p2pd." (number->string (getpid)) ".sock")))
           ...
           ;; 1. Why do you need sh?
           ;; 2. Does `open-process` fork?
           (_ (DBG cmd))
           (proc (begin (##trace open-process)
                        (open-process [path: "/bin/sh" arguments: ["-c" cmd]])))
           ;; sh -c exec "p2pd ..."
